...
var PONTOS;
var RESPOSTA;
var estado; // 'jogando' ou 'incorreto'
var resposta_correta_str;
...
function draw() {
	...
	if( comparacao != 'X' ){
		...
		text( comparacao,  Gx + hGr * E, Gy + hGc * E );
		pop();
	}
...
function mouseReleased(){
	if (estado === 'incorreto') {
		let btnContinuar = { x: width/2 - 100, y: height/2 + 100, w: 200, h: 50 };
		if (coords_in_rct(mouseX, mouseY, btnContinuar)) {
			estado = 'jogando';
			next_question();
		}
		return;
	...
	if( draw_keyboard == VouF_keyboard ){
		...
		if( coords_in_rct( mouseX, mouseY, R ) ){
			if( RESPOSTA == true ){
				...
				estado = 'incorreto';
				resposta_correta_str = RESPOSTA ? "Verdadeiro" : "Falso";
			}
		}
		else{
			R.y += kbr.h / 2;
		...
		if( I == RESPOSTA ){
			...
			estado = 'incorreto';
			resposta_correta_str = "Opção " + (RESPOSTA + 1);
		}
	}
}

function draw_incorreto() {
	...
	textSize(60);
	textAlign(CENTER, CENTER);
	text("Incorreto!", width/2, height/2 - 50);
	
	// Resposta correta
	textSize(30);
	text("Resposta correta: " + resposta_correta_str, width/2, height/2);
	...
	stroke(0);
	strokeWeight(3);
	rect(btnContinuar.x, btnContinuar.y, btnContinuar.w, btnContinuar.h, 10);
	
	fill(255);
	noStroke();
	textSize(30);
	text("Continuar", width/2, btnContinuar.y + btnContinuar.h/2);
}
...
function next_question(){
	estado = 'jogando'; // Resetar estado ao começar nova pergunta
	...
	switch( q_type ){
		...
		case 0:{ // [] > [] (verdadeiro / falso)
			...
			for (var i = 0; i < 2; i++) {
				...
				if( i == 1 && comparacao == '=' && RESPOSTA == true ){
